...
class TokenManager
{
    ...
    public function createCompositionToken(User $user): CompositionToken
    {
        ...
        $this->cache->rememberForever("app.composition-tokens.$token->apiToken", static fn () => $token->apiToken);
    ...
    {
        $apiToken = $this->cache->get("app.composition-tokens.$plainTextApiToken");

        if ($apiToken) {
            self::deleteTokenByPlainTextToken($apiToken);
